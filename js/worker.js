...
async function handleIcon(request, sender){
  let compsFound = compList(request.message.resourceList); 
  let isCompetitorDetected = Object.keys(compsFound).length > 0; 
  const isDetected = request.message.status || isCompetitorDetected;
  const color = isDetected ? '#50fa7b' : '#ff5555';
  const text = isDetected ? 'YES' : 'NO';
  const icons = isDetected ? {
            //16: "/assets/icons/color/16.png",
           // 32: "/assets/icons/color/32.png",
            48: "/assets/icons/color/48.png",
            48: "/assets/icons/Competitor/Nosto48.png"
